# setup-admin.ps1
# Complete LittleLoom Admin Setup Script

Write-Host "🧵 LittleLoom Admin Console Setup" -ForegroundColor Cyan
Write-Host "=================================" -ForegroundColor Cyan
Write-Host ""

# Create directories
$adminDir = "admin"
$pagesDir = "$adminDir\pages"
$cssDir = "$adminDir\css"
$jsDir = "$adminDir\js"
...
Write-Host "✅ CSS created" -ForegroundColor Green

# Create JS
Write-Host "📜 Creating JavaScript..." -ForegroundColor Yellow
@"
// ─── SUPABASE CONFIG ──────────────────────────────────────────────
...
Write-Host "✅ JavaScript created" -ForegroundColor Green
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="sidebar-nav" id="sidebarNav">
            ...
            <a class="sidebar-nav-item" data-page="realtime" href="#" onclick="loadPage('realtime')">
                <span class="icon">🔄</span> Realtime
                <span class="badge success">Live</span>
            </a>
            <a class="sidebar-nav-item" data-page="health" href="#" onclick="loadPage('health')">
                <span class="icon">❤️</span> System Health
                <span class="badge success" id="healthBadge">OK</span>
            </a>
            ...
            <a class="sidebar-nav-item" data-page="notifications" href="#" onclick="loadPage('notifications')">
                <span class="icon">🔔</span> Push Notifications
            ...
            <a class="sidebar-nav-item" data-page="support" href="#" onclick="loadPage('support')">
                ...
                <span class="badge danger" id="supportBadge">0</span>
            </a>
            ...
            <div class="iframe-container" id="iframeContainer">
                <div class="page-loading" id="pageLoading">
                    <div>
                        ...
                        <div style="margin-top:12px;">Loading page...</div>
                    </div>
                </div>
                <iframe id="pageFrame" style="display:none;" frameborder="0" sandbox="allow-scripts allow-same-origin"></iframe>
            </div>
        </div>
    </main>
    ...
    <script>
        function loadPage(page) {
            ...
            if (page === 'dashboard') {
                ...
                iframe.style.display = 'none';
                window.location.href = '/admin/dashboard-content.html';
                return;
            }
...
"@ | Out-File -FilePath "$adminDir\dashboard.html" -Encoding UTF8
Write-Host "✅ dashboard.html created" -ForegroundColor Green

# Create page files
Write-Host "📄 Creating page files..." -ForegroundColor Yellow

$pageFiles = @(
    @{Name="babies.html"; Title="Babies Management"},
    @{Name="users.html"; Title="User Management"},
    @{Name="moderation.html"; Title="Content Moderation"},
    @{Name="community.html"; Title="Community Management"},
    @{Name="topics.html"; Title="Topic Management"},
    @{Name="trackers.html"; Title="Tracker Management"},
    @{Name="milestones.html"; Title="Milestone Management"},
    @{Name="analytics.html"; Title="Analytics Dashboard"},
    @{Name="performance.html"; Title="Performance Metrics"},
    @{Name="realtime.html"; Title="Realtime Events"},
    @{Name="health.html"; Title="System Health"},
    @{Name="audit.html"; Title="Audit Logs"},
    @{Name="notifications.html"; Title="Push Notifications"},
    @{Name="features.html"; Title="Feature Flags"},
    @{Name="export.html"; Title="Data Export"},
    @{Name="api.html"; Title="API Management"},
    @{Name="support.html"; Title="Support Tickets"},
    @{Name="announcements.html"; Title="Announcements"},
    @{Name="settings.html"; Title="System Settings"},
    @{Name="backup.html"; Title="Backup & Restore"}
)

foreach ($page in $pageFiles) {
    $pagePath = "$pagesDir\$($page.Name)"
    @"
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>$($page.Title) - LittleLoom Admin</title>
    <script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2.39.0/dist/umd/supabase.min.js"></script>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800;900&display=swap" rel="stylesheet" />
    <link rel="stylesheet" href="/admin/css/admin.css" />
    <style>
...
<body>
    <div class="page-container">
        ...
        <div class="status-bar" id="statusBar">
            <span>✅ Ready</span>
            <span style="margin-left:auto;font-size:12px;" id="lastUpdated">Last updated: <span id="updateTime">—</span></span>
        </div>
        <div class="card">
            <div class="card-body">
                <div style="text-align:center;padding:40px 20px;color:var(--text-muted);">
                    <div style="font-size:48px;margin-bottom:12px;">🚀</div>
                    <h3 style="color:var(--text-primary);font-weight:700;margin-bottom:6px;">$($page.Title)</h3>
                    <p>Full functionality is being loaded from Supabase. Please wait...</p>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Load data from Supabase based on page type
            const page = '$($page.Name)'.replace('.html', '');
            loadPageData(page);
        });
        ...
        async function loadPageData(page) {
            ...
            showToast('Loading $($page.Title)...', 'info');
            // Page-specific data loading will be implemented
        }
    </script>
</body>
</html>
...
Write-Host ""
Write-Host "🎉 Admin Console Setup Complete!" -ForegroundColor Green
Write-Host ""
Write-Host "📁 Files created:" -ForegroundColor Cyan
Write-Host "   - admin/dashboard.html" -ForegroundColor White
Write-Host "   - admin/css/admin.css" -ForegroundColor White
Write-Host "   - admin/js/admin.js" -ForegroundColor White
Write-Host "   - admin/pages/ (20 page files)" -ForegroundColor White
Write-Host ""
...
Write-Host ""
Write-Host "📱 To test on mobile (same WiFi):" -ForegroundColor Yellow
Write-Host "   Find your IP: ipconfig | findstr IPv4" -ForegroundColor White
Write-Host "   Open: http://YOUR_IP:3000/admin/dashboard.html" -ForegroundColor White