# fix-dashboard.ps1
Write-Host "🔧 Fixing dashboard..." -ForegroundColor Cyan
    ...
    <script>
        ...
        function setupRealtime() {
            ...
            realtimeChannel = supabase.channel('admin-realtime')
                .on('postgres_changes', { event: '*', schema: 'public', table: 'babies' }, function(p) { 
                    fetchDashboardData(); 
                })
                .on('postgres_changes', { event: '*', schema: 'public', table: 'tracker_entries' }, function(p) { 
                    fetchDashboardData(); 
                })
                .on('postgres_changes', { event: '*', schema: 'public', table: 'community_posts' }, function(p) { 
                    fetchDashboardData(); 
                })
...
"@ | Out-File -FilePath "admin\dashboard.html" -Encoding UTF8
