...
export const BabyProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const mapTrackerEntryToDomain = useCallback((row: any) => {
    ...
    // Use tracker_type or fallback to tracker_id for compatibility
  ...
  const loadBabies = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        if (error1) {
          if (error1.message?.includes('infinite recursion')) {
            console.error('[BabyContext] RLS RECURSION ERROR - Fix your RLS policies!');
            hadRlsError = true;
          } else {
      ...
      // If no babies as parent1 and no RLS error, try parent2
      ...
      // If RLS error, try fallback without is_active
      if (hadRlsError && allBabies.length === 0) {
        try {
          ...
          console.warn('[BabyContext] Fallback query failed:', e);
        }
      }
...
const createBaby = useCallback(async (
  ...
  try {
    ...
    // Check for duplicate - use userId
    ...
    // Insert baby - using userId directly
  ...
  }, [loadAllBabyData, mapBabyRowToProfile]);
  ...
  }, [state.currentBabyId, mapBabyRowToProfile, loadAllBabyData, loadBabies]);
  ...
  const addActivity = useCallback(async (entry: Omit<ActivityEntry, 'id'>): Promise<boolean> => {
    ...
    try {
      ...
      // Extract data fields (all fields except the base ones)
  ...
  const deleteActivity = useCallback(async (id: string): Promise<boolean> => {
    try {
      ...
      // Remove from ActivityContext storage
      try {
        const existing = await AsyncStorage.getItem(ACTIVITY_CONTEXT_KEY);
        if (existing) {
  ...
  const updateEntry = useCallback(async (id: string, updates: Partial<ActivityEntry>): Promise<boolean> => {
    try {
      ...
      // Update in ActivityContext storage
      try {
        const existing = await AsyncStorage.getItem(ACTIVITY_CONTEXT_KEY);
        if (existing) {
          const entries: ActivityEntry[] = JSON.parse(existing);
