...
export function useDashboardIntelligence(): DashboardIntelligence {
  ...
  const { reminders } = usePredictiveReminders();
  const { correlations } = useTimelineCorrelations();
  ...
  const todayEntries = useMemo(() => 
    allEntries.filter(e => e.timestamp >= todayStart),
    [allEntries, todayStart]
  );

  const weekEntries = useMemo(() =>
    allEntries.filter(e => e.timestamp >= weekAgo),
    [allEntries, weekAgo]
  );
  ...
  const streakDays = useMemo(() => {
    ...
    while (true) {
      ...
      const hasEntry = allEntries.some(e => e.timestamp >= dayStart && e.timestamp <= dayEnd);
      if (hasEntry) {
        streak++;
        date = subDays(date, 1);
      } else {
  ...
  const nextEvents = useMemo(() => {
    ...
    // Feed prediction
    const feedEntries = allEntries.filter(e => e.trackerId === 'feed').sort((a, b) => b.timestamp - a.timestamp);
    if (feedEntries.length >= 2) {
      const avgGap = (feedEntries[0].timestamp - feedEntries[Math.min(3, feedEntries.length - 1)].timestamp) / 
    ...
    // Sleep prediction
    const sleepEntries = allEntries.filter(e => e.trackerId === 'sleep').sort((a, b) => b.timestamp - a.timestamp);
    ...
    // Diaper prediction
    const diaperEntries = allEntries.filter(e => e.trackerId === 'diaper').sort((a, b) => b.timestamp - a.timestamp);
    if (diaperEntries.length >= 2) {
      const avgGap = (diaperEntries[0].timestamp - diaperEntries[Math.min(5, diaperEntries.length - 1)].timestamp) / 
