...
export const useTrackerProgressive = (trackerId: string) => {
  ...
  const { prefillData, suggestions } = useMemo(() => {
    ...
    const NEVER_PREFILL = new Set([
      ...
      'time', 'bedtime', 'wakeTime', 'datetime',
    ]);

    const isNeverPrefill = (fieldId: string) => {
      const lower = fieldId.toLowerCase();
      return (
        NEVER_PREFILL.has(fieldId) ||
        lower.endsWith('time') ||
        lower.endsWith('date') ||
    ...
    if (partnerEntry) {
      const partnerTimeAgo = Math.round(
        (now.getTime() - partnerEntry.timestamp) / 60000
      );
      const timeLabel =
        partnerTimeAgo < 60
          ? `${partnerTimeAgo}m ago`
        ...
        if (
          ...
          if (validated) {
            suggMap.set(fieldId, {
              ...validated,
              source: 'partner',
              label: `Co-parent ${timeLabel}: ${String(value).slice(0, 16)}`,
              emoji: '👥',
            });
          }
        }
    ...
    safeCorrelations
      ...
      .forEach((c) => {
        ...
        if (trackerId === 'sleep' && c.type === 'feed_sleep_pattern') {
          if (!suggMap.has('sleepType')) {
            suggMap.set('sleepType', {
              fieldId: 'sleepType',
              value: 'nap',
              source: 'correlation',
              confidence: 70,
              label: 'Post-feed nap pattern',
              emoji: '🍼😴',
            });
          }
        }
    ...
    for (const sugg of suggMap.values()) {
      ...
      if (!existing) {
        byField.set(sugg.fieldId, sugg);
        continue;
      }
      const existingRank = sourceRank[existing.source] ?? 0;
      const newRank = sourceRank[sugg.source] ?? 0;
      if (
        newRank > existingRank ||
        (newRank === existingRank && sugg.confidence > existing.confidence)
      ) {
        byField.set(sugg.fieldId, sugg);
      }
    }
  ...
  const trends = useMemo(() => {
    ...
    return result;
  }, [todayEntries, yesterdayEntries, prefillData, trackerFingerprint, trackerId]);
  ...
  const insights = useMemo(() => {
    ...
    return filtered;
  }, [trackerFingerprint, trackerId, refreshToken, contextTick, aiFingerprint]);
      ...
      .map((c) => {
        ...
        if (trackerId === 'medication' && c.type === 'health_alert') {
          ...
          prefillData = lastMed
            ? {
                reason: 'Fever',
                ...(lastMed.data?.name ? { name: lastMed.data.name } : {}),
                ...(lastMed.data?.dosage ? { dosage: lastMed.data.dosage } : {}),
              }
            : { reason: 'Fever' };
  ...
  }, [timelineCorrelations, aiCorrelations, trackerId, safeTrackers, trackerFingerprint, refreshToken, aiFingerprint]);
  ...
  const activeReminders = useMemo((): ProgressiveReminder[] => {
    ...
    return all;
  }, [trackerFingerprint, predictiveReminders, trackerId, refreshToken]);
  ...
  const templates = useMemo((): ProgressiveTemplate[] => {
    ...
    if (
      ...
      if (lastFeverMed) {
        autoGenerated.push({
          ...
          name: String(lastFeverMed.data?.name || 'Fever medication'),
          emoji: '🌡️',
          data: {
            ...(lastFeverMed.data?.name ? { name: lastFeverMed.data.name } : {}),
            ...(lastFeverMed.data?.dosage ? { dosage: lastFeverMed.data.dosage } : {}),
            ...(lastFeverMed.data?.type ? { type: lastFeverMed.data.type } : {}),
            reason: 'Fever',
          },
    ...
    if (
      ...
      const modalLocation = Object.entries(locationCounts).sort((a, b) => b[1] - a[1])[0]?.[0];
      autoGenerated.push({
        id: 'auto_nap',
        name: 'Post-Feed Nap',
        emoji: '😴',
  ...
  }, [trackerConfig, userSavedTemplates, trackerId, correlations, trackerFingerprint, refreshToken]);
