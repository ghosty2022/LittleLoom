...
import { format, isToday, isYesterday, isThisWeek, isThisMonth, subDays, parseISO, differenceInMonths } from 'date-fns';
...
interface Photo {
  ...
  date: string;
  timestamp: number;
  type: 'milestone' | 'daily' | 'sleep' | 'feeding' | 'potty' | 'growth' | 'medication' | 'all';
  caption?: string;
  babyId?: string;
  babyName?: string;
  isPrivate?: boolean;
...
const SweetAlert = ({ 
  ...
  isDark,
  duration = 3000
}: AlertState & { onClose: () => void; isDark: boolean; duration?: number }) => {
  ...
  return (
    <View style={[StyleSheet.absoluteFill, { zIndex: 99999, alignItems: 'center', justifyContent: 'flex-start', paddingTop: 100, pointerEvents: 'none' }]} pointerEvents="box-none">
      <AnimatedReanimated.View style={[style, styles.alertContainer, { backgroundColor: isDark ? '#1a1a2e' : '#fff' }]}>
...
const ConfirmModal = ({ 
  ...
  return (
    <View style={[StyleSheet.absoluteFill, { zIndex: 100000, justifyContent: 'center', alignItems: 'center' }]} pointerEvents="auto">
      <TouchableOpacity 
        activeOpacity={1} 
        onPress={onCancel}
        style={StyleSheet.absoluteFill}
      >
        <AnimatedReanimated.View style={[StyleSheet.absoluteFill, { backgroundColor: 'rgba(0,0,0,0.6)' }, backdropStyle]}>
          <BlurView intensity={80} style={StyleSheet.absoluteFill} tint="dark" />
        </AnimatedReanimated.View>
      </TouchableOpacity>
      
      <AnimatedReanimated.View style={[styles.confirmModal, modalStyle, { backgroundColor: isDark ? '#1a1a2e' : '#fff' }]}>
...
// ==================== VAULT LOCK MODAL - USES EXISTING SECURITY ====================
...
const VaultLockModal = ({ 
  ...
  useEffect(() => {
    if (visible) {
      ...
      const hasSecurity = securitySettings.isPinEnabled || 
                         (securitySettings.isBiometricEnabled && isBiometricHardwareAvailable && isBiometricEnrolled);
      ...
      } else if (securitySettings.isBiometricEnabled && isBiometricHardwareAvailable && isBiometricEnrolled) {
        // Try biometric first
        attemptBiometric();
      } else {
    ...
    <Modal visible={visible} transparent animationType="fade">
      <View style={[StyleSheet.absoluteFill, { backgroundColor: isDark ? '#000' : '#fff' }]}>
        ...
        <View style={styles.vaultLockContainer}>
          <LinearGradient colors={['#ef4444', '#dc2626']} style={styles.vaultLockIconBg}>
            <Ionicons name="shield" size={48} color="#fff" />
          </LinearGradient>
          
          <Text style={[styles.vaultLockTitle, { color: isDark ? '#fff' : '#1e293b' }]}>
            Private Vault
          </Text>
          ...
          <View style={styles.pinContainer}>
            {[0, 1, 2, 3].map((i) => (
              <View key={i} style={[styles.pinDot, pin.length > i && styles.pinDotFilled, styles.pinDotVault]} />
            ))}
          </View>
          ...
          {showPinEntry ? (
            <View style={styles.keypad}>
              {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((num) => (
                ...
                >
                  <Text style={[styles.keypadNumber, { color: isDark ? '#fff' : '#1e293b' }]}>{num}</Text>
                </TouchableOpacity>
              ))}
              <TouchableOpacity style={styles.keypadButton} onPress={attemptBiometric}>
              ...
              >
                <Ionicons name="backspace" size={28} color={isDark ? '#fff' : '#1e293b'} />
              </TouchableOpacity>
            </View>
          ) : (
            <TouchableOpacity style={styles.biometricPrompt} onPress={attemptBiometric}>
              <Ionicons name="finger-print" size={64} color="#ef4444" />
              <Text style={styles.biometricPromptText}>Tap to authenticate</Text>
            </TouchableOpacity>
          )}

          <TouchableOpacity style={styles.cancelVaultButton} onPress={onCancel}>
...
const ExplicitContentDetector = {
  async analyzeImage(uri: string): Promise<{ isExplicit: boolean; confidence: number; reasons: string[] }> {
    try {
      const fileInfo = await FileSystem.getInfoAsync(uri);
      if (!fileInfo.exists) return { isExplicit: false, confidence: 0, reasons: [] };
  ...
  async detectFaces(uri: string): Promise<Array<{
    boundingBox: { x: number; y: number; width: number; height: number };
    confidence: number;
    features: number[];
  }>> {
  ...
  async matchBabyFace(faceFeatures: number[], babySignature: string): Promise<number> {
...
const AutoImportManager = {
  async scanDeviceGallery(
    ...
    while (hasMore && scanned < 1000) {
      ...
      for (const asset of result.assets) {
        ...
        if (shouldImport) {
          const explicitCheck = await ExplicitContentDetector.analyzeImage(asset.uri);
  ...
  async saveSettings(settings: any): Promise<void> {
    await AsyncStorage.setItem(STORAGE_KEYS.AUTO_IMPORT_SETTINGS, JSON.stringify(settings));
  },

  async loadSettings(): Promise<any> {
    const saved = await AsyncStorage.getItem(STORAGE_KEYS.AUTO_IMPORT_SETTINGS);
  ...
  async saveScanProgress(progress: any): Promise<void> {
    await AsyncStorage.setItem(STORAGE_KEYS.SCAN_PROGRESS, JSON.stringify({
      ...progress,
      timestamp: Date.now(),
    }));
  },
  ...
  async loadScanProgress(): Promise<any | null> {
    ...
    // Check if progress is from current session (within last 24 hours)
...
const PhotoEditorModal = ({
  ...
  const filters = [
    { id: 'normal', name: 'Normal', icon: 'sunny', colors: ['#transparent', '#transparent'] },
    { id: 'warm', name: 'Warm', icon: 'flame', colors: ['#ff9a56', '#ffad56'] },
    { id: 'cool', name: 'Cool', icon: 'snow', colors: ['#56c1ff', '#56d2ff'] },
    { id: 'bw', name: 'B&W', icon: 'contrast', colors: ['#333', '#666'] },
    { id: 'vintage', name: 'Vintage', icon: 'time', colors: ['#d4a574', '#c49a6c'] },
    { id: 'bright', name: 'Bright', icon: 'sunny-outline', colors: ['#fff9c4', '#fff59d'] },
    { id: 'soft', name: 'Soft', icon: 'water', colors: ['#fce4ec', '#f8bbd0'] },
    { id: 'crisp', name: 'Crisp', icon: 'aperture', colors: ['#e0f7fa', '#b2ebf2'] },
  ];
  ...
  const applyEdit = async () => {
    ...
    try {
      ...
      onSave(processed.uri, { filter: activeFilter, brightness, contrast, saturation });
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
    } catch (error) {
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error);
    } finally {
  ...
  return (
    <Modal visible={visible} animationType="slide" transparent={false}>
      <View style={[styles.editorContainer, { backgroundColor: isDark ? '#000' : '#fff' }]}>
        <View style={styles.editorHeader}>
          <TouchableOpacity onPress={onClose}>
            <Ionicons name="close" size={28} color={isDark ? '#fff' : '#1e293b'} />
          </TouchableOpacity>
          <Text style={[styles.editorTitle, { color: isDark ? '#fff' : '#1e293b' }]}>Edit Photo</Text>
        ...
        <View style={styles.editorImageContainer}>
          ...
          {activeFilter !== 'normal' && (
            <View style={[StyleSheet.absoluteFill, { 
              backgroundColor: filters.find(f => f.id === activeFilter)?.colors[0],
              opacity: 0.3,
            }]} />
          )}
        </View>
        ...
        <View style={styles.adjustmentsContainer}>
          <View style={styles.adjustmentRow}>
            ...
            <Slider 
              ...
              label="Brightness"
            />
            <Text style={styles.adjustmentValue}>{brightness > 0 ? `+${brightness}` : brightness}</Text>
          </View>
          ...
          <View style={styles.adjustmentRow}>
            ...
            <Slider 
              ...
              label="Contrast"
            />
            <Text style={styles.adjustmentValue}>{contrast > 0 ? `+${contrast}` : contrast}</Text>
          </View>
          ...
          <View style={styles.adjustmentRow}>
            ...
            <Slider 
              ...
              label="Saturation"
            />
            <Text style={styles.adjustmentValue}>{saturation > 0 ? `+${saturation}` : saturation}</Text>
          </View>
        </View>
...
const Slider = ({ value, onValueChange, min, max, label }: { value: number; onValueChange: (val: number) => void; min: number; max: number; label: string }) => (
...
const BatchSelectBar = ({
  ...
  return (
    <AnimatedReanimated.View 
      entering={FadeInUp}
      exiting={FadeInDown}
      style={[styles.batchBar, { backgroundColor: isDark ? '#1a1a2e' : '#fff' }]}
    >
      <View style={styles.batchInfo}>
        <Text style={[styles.batchCount, { color: isDark ? '#fff' : '#1e293b' }]}>
      ...
      <View style={styles.batchActions}>
        {onCreateStory && (
          <TouchableOpacity style={styles.batchButton} onPress={onCreateStory}>
...
const SmartAlbumsSection = ({
  ...
  return (
    <View style={styles.smartAlbumsContainer}>
      <Text style={[styles.sectionTitle, { color: isDark ? '#fff' : '#1e293b' }]}>Smart Albums</Text>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.albumsScroll}>
        {albums.map((album, index) => (
          ...
          >
            ...
            >
              <LinearGradient colors={album.gradient} style={styles.albumGradient}>
                <View style={styles.albumIconContainer}>
                  <Ionicons name={album.icon as any} size={28} color="#fff" />
                </View>
                <Text style={styles.albumCount}>{album.count}</Text>
              </LinearGradient>
              <Text style={[styles.albumTitle, { color: isDark ? '#fff' : '#1e293b' }]} numberOfLines={1}>
                {album.title}
              </Text>
            </TouchableOpacity>
          </AnimatedReanimated.View>
...
const FolderGrid = ({
  ...
  return (
    <View style={styles.folderContainer}>
      <Text style={[styles.sectionTitle, { color: isDark ? '#fff' : '#1e293b' }]}>Folders</Text>
      <View style={styles.folderGrid}>
        {folders.map((folder, index) => (
          ...
          >
            ...
            >
              <LinearGradient colors={folder.gradient} style={styles.folderGradient}>
                <View style={styles.folderIconContainer}>
                  <Ionicons name={folder.icon as any} size={32} color="#fff" />
                </View>
                <View style={styles.folderOverlay}>
                  <Text style={styles.folderCount}>{folder.count}</Text>
                </View>
              </LinearGradient>
              <Text style={[styles.folderTitle, { color: isDark ? '#fff' : '#1e293b' }]} numberOfLines={1}>
                {folder.title}
              </Text>
...
const SearchFilterBar = ({
  ...
  const filters = ['All', 'Favorites', 'Private', 'Milestones', 'Screenshots', 'Auto-Imported'];
...
const AutoImportPanel = ({
  ...
  return (
    <View style={[styles.importPanel, { backgroundColor: isDark ? 'rgba(102,126,234,0.1)' : 'rgba(102,126,234,0.05)' }]}>
      <View style={styles.importHeader}>
        <View style={styles.importIconBg}>
          <Ionicons name="sync" size={24} color="#667eea" />
        </View>
        <View style={styles.importInfo}>
          <Text style={[styles.importTitle, { color: isDark ? '#fff' : '#1e293b' }]}>
            Auto-Import Photos
          </Text>
          <Text style={styles.importSubtitle}>
            {lastScanTime ? `Last scan: ${format(lastScanTime, 'MMM d, h:mm a')}` : 'Scanning for new photos...'}
          </Text>
        </View>
      ...
      {isScanning && (
        <View style={styles.scanProgress}>
          ...
          <Text style={styles.scanText}>
            Scanned {scanProgress.scanned} photos • Found {scanProgress.found} matches
          </Text>
        </View>
      )}
      ...
      <View style={styles.importOptions}>
        <View style={styles.importOption}>
          <Text style={[styles.importOptionText, { color: isDark ? '#cbd5e1' : '#64748b' }]}>
            Baby Face Detection
          </Text>
          <Switch
      ...
      <View style={styles.importActions}>
        <TouchableOpacity style={styles.scanButton} onPress={onManualScan} disabled={isScanning}>
...
const DateGroupHeader = ({ date, isDark }: { date: string; isDark: boolean }) => {
  const getGroupLabel = () => {
  ...
  return (
    <View style={styles.dateGroupHeader}>
      <Text style={[styles.dateGroupText, { color: isDark ? '#94a3b8' : '#64748b' }]}>
        {getGroupLabel()}
      </Text>
...
export default function GalleryScreen({ navigation }: GalleryScreenProps) {
  ...
  const { currentBaby, babies, switchBaby, growthData, milestones, addActivity } = useBaby();
  const { addActivity: addTimelineActivity } = useActivity();
  const { 
    isBiometricHardwareAvailable, 
    authenticateWithBiometric, 
    settings: securitySettings,
  ...
  const [activeAlbum, setActiveAlbum] = useState<SmartAlbum | null>(null);
  
  const [autoImportEnabled, setAutoImportEnabled] = useState(true); // Always on
  ...
  useEffect(() => {
    const loadSavedData = async () => {
      try {
        ...
        setAutoImportEnabled(settings.enabled !== false); // Default to true
        setBabyFaceDetection(settings.babyFaceDetection !== false);
        setLastScanTime(settings.lastScanTime || 0);

        // Load vault state
        const vaultState = await AsyncStorage.getItem(STORAGE_KEYS.VAULT_ENABLED);
        ...
        if (savedProgress && !savedProgress.completed) {
          // Resume scan if it was interrupted
          showToast('info', 'Resuming Scan', 'Continuing from where we left off...');
          handleManualScan();
        }
      } catch (error) {
  ...
  useEffect(() => {
    const init = async () => {
      // Auto-import is always on, start scanning if we have a baby
  ...
  // ==================== SMART ALBUMS & FOLDERS GENERATION ====================

  const generateSmartAlbums = useCallback((photoList: Photo[]) => {
    const now = Date.now();
    const today = photoList.filter(p => isToday(p.timestamp));
    const thisWeek = photoList.filter(p => isThisWeek(p.timestamp));
    ...
    const autoImported = photoList.filter(p => p.source === 'auto_import');
  ...
  const generateFolders = useCallback((photoList: Photo[]) => {
    ...
    const generatedFolders: SmartAlbum[] = Array.from(folderMap.entries()).map(([name, photos], index) => ({
      id: `folder_${name}`,
      title: name.charAt(0).toUpperCase() + name.slice(1),
      description: `${photos.length} photos`,
      icon: photos[0]?.source === 'google_photos' ? 'logo-google' : 
            photos[0]?.source === 'icloud' ? 'cloud' : 'folder',
      count: photos.length,
      photos,
      type: 'folder',
  ...
  const handleManualScan = async () => {
    ...
    try {
      ...
      // Process any remaining photos that weren't caught by real-time
      const newPhotos = importedPhotos.filter(p => !photos.some(existing => existing.id === p.id));
      
      if (newPhotos.length > 0) {
        setPhotos(prev => [...newPhotos, ...prev]);
        showToast('success', 'Scan Complete', `Found ${newPhotos.length} new photos`);
      } else {
      ...
      settings.enabled = true; // Always keep enabled
      ...
      generateSmartAlbums([...importedPhotos, ...photos]);
      generateFolders([...importedPhotos, ...photos]);
    } catch (error) {
  ...
  }, [photos, selectedCategory, activeAlbum, searchQuery, selectedFilter, vaultEnabled, vaultUnlocked]);

  // ==================== TOAST & ALERTS ====================

  const showToast = useCallback((type: AlertState['type'], title: string, message: string, duration?: number) => {
  ...
  const togglePrivate = useCallback(async (photoId: string) => {
    if (!vaultEnabled) {
      // Check if security is set up
      const hasSecurity = securitySettings.isPinEnabled || 
                         (securitySettings.isBiometricEnabled && isBiometricHardwareAvailable);
      ...
      await AsyncStorage.setItem(STORAGE_KEYS.VAULT_ENABLED, 'true');
    }
    ...
    Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
    showToast('success', 'Updated', photoId ? 'Photo moved to vault' : 'Photo removed from vault');
  }, [vaultEnabled, securitySettings, isBiometricHardwareAvailable, navigation, showToast]);

  const handleDelete = useCallback((photoIds: string[]) => {
    showConfirm(
      photoIds.length === 1 ? 'Delete Photo' : `Delete ${photoIds.length} Photos`,
      photoIds.length === 1 
      ...
      async () => {
        ...
        for (const photoId of photoIds) {
          ...
          if (photo && photo.uri.startsWith(FileSystem.documentDirectory || '')) {
            try {
        ...
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
        showToast('success', 'Deleted', photoIds.length === 1 ? 'Photo deleted' : `${photoIds.length} photos deleted`);
      },
      'danger'
    );
  ...
  const handleShare = useCallback(async (photo: Photo) => {
    try {
      const shareMessage = `Check out this photo of ${currentBaby?.name || 'baby'}! ${photo.caption || ''}\n\nShared via LittleLoom - Secure Family Gallery`;
  ...
  const handleDownload = useCallback(async (photoIds: string[]) => {
    try {
      const { status } = await MediaLibrary.requestPermissionsAsync();
      if (status !== 'granted') {
        showToast('error', 'Permission Denied', 'Please allow access to save photos');
        return;
      }
      ...
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      showToast('success', 'Saved', `${photoIds.length} photo(s) saved to library`);
    } catch (error) {
  ...
  const handleSaveEdit = useCallback((editedUri: string, adjustments: any) => {
    setPhotos(prev => prev.map(p => 
  ...
  const handleLinkToEntry = useCallback((photo: Photo, entryType: 'milestone' | 'growth' | 'activity', entryId: string) => {
    setPhotos(prev => prev.map(p => 
      p.id === photo.id ? { 
        ...p, 
        linkedEntry: { type: entryType, id: entryId, title: 'Linked Entry' } 
      } : p
    ));
  ...
  const handleCreateStory = useCallback((photoIds: string[]) => {
  ...
  const handleOpenCamera = useCallback(async () => {
    try {
      ...
      if (uri) {
        const explicitCheck = await ExplicitContentDetector.analyzeImage(uri);
        if (explicitCheck.isExplicit) {
          showToast('warning', 'Content Warning', 'This photo may contain sensitive content and was not saved');
          return;
        }
        ...
        generateSmartAlbums([newPhoto, ...photos]);
        generateFolders([newPhoto, ...photos]);
        showToast('success', 'Photo Captured', 'Saved to gallery');
      }
    } catch (error) {
      showToast('error', 'Error', 'Failed to capture photo');
    }
  }, [takePhoto, currentBaby, photos, generateSmartAlbums, generateFolders, showToast]);
  ...
  const navigateToGrowthChart = useCallback(() => {
  ...
  const renderPhoto = useCallback(({ item, index }: { item: Photo; index: number }) => {
    ...
    return (
      <AnimatedReanimated.View entering={FadeInUp.delay(index * 30)} layout={Layout.springify()}>
        <TouchableOpacity 
          style={[
        ...
        >
          ...
          {viewMode === 'list' && (
            <View style={styles.listInfo}>
              <Text style={styles.listDate}>{format(item.timestamp, 'MMM d, yyyy')}</Text>
              {item.caption && (
  ...
  }, [viewMode, isBatchMode, selectedPhotos, handlePhotoPress, handlePhotoLongPress]);

  // Group photos by date for SectionList
              ...
              <View style={styles.infoContent}>
                  ...
                  <View style={styles.typeBadge}>
                    <Ionicons 
                      name={selectedPhoto.type === 'milestone' ? 'trophy' : 
                            selectedPhoto.type === 'sleep' ? 'moon' : 
                            selectedPhoto.type === 'feeding' ? 'restaurant' :
                            selectedPhoto.type === 'growth' ? 'trending-up' :
                            selectedPhoto.type === 'potty' ? 'water' :
                            selectedPhoto.type === 'medication' ? 'medical' : 'camera'} 
                      size={16} 
                      color="#667eea" 
                    />
                ...
                {selectedPhoto.linkedEntry && (
                  <TouchableOpacity style={styles.linkedEntryCard}>
                    <LinearGradient colors={['#667eea', '#764ba2']} style={styles.linkedEntryIcon}>
                      <Ionicons name="link" size={20} color="#fff" />
                    </LinearGradient>
                    <View style={styles.linkedEntryInfo}>
                      <Text style={styles.linkedEntryTitle}>{selectedPhoto.linkedEntry.title}</Text>
                ...
                {selectedPhoto.source && (
                  <View style={styles.sourceInfo}>
                    <Ionicons 
                      name={selectedPhoto.source === 'camera' ? 'camera' : 
                            selectedPhoto.source === 'auto_import' ? 'sync' : 
                            selectedPhoto.source === 'google_photos' ? 'logo-google' :
                            selectedPhoto.source === 'icloud' ? 'cloud' : 'images'} 
                      size={14} 
                      color="#94a3b8" 
                    />
                    <Text style={styles.sourceText}>
                      {selectedPhoto.source === 'camera' ? 'Camera Capture' : 
                       selectedPhoto.source === 'auto_import' ? 'Auto-Imported' : 
                       selectedPhoto.source === 'google_photos' ? 'Google Photos' :
                       selectedPhoto.source === 'icloud' ? 'iCloud Photos' : 'Gallery Import'}
                    </Text>
                  </View>
                )}
                ...
                <View style={styles.actionButtons}>
                  ...
                  >
                    <View style={[styles.actionButtonGradient, { backgroundColor: '#10b981' }]}>
                  ...
                  >
                    <View style={[styles.actionButtonGradient, { backgroundColor: '#ef4444' }]}>
                  ...
                  <View style={styles.exifContainer}>
                    ...
                    <View style={styles.exifGrid}>
                      <View style={styles.exifItem}>
                        ...
                        <Text style={styles.exifValue}>{selectedPhoto.exif.width}×{selectedPhoto.exif.height}</Text>
                      </View>
                      <View style={styles.exifItem}>
                        <Ionicons name="document" size={16} color="#64748b" />
                        <Text style={styles.exifValue}>{(selectedPhoto.exif.size / 1024 / 1024).toFixed(2)} MB</Text>
                      </View>
                      <View style={styles.exifItem}>
                        <Ionicons name="phone-portrait" size={16} color="#64748b" />
                        <Text style={styles.exifValue}>{selectedPhoto.exif.device}</Text>
                      </View>
                    </View>
                  </View>
                )}
                ...
                <View style={styles.backupStatus}>
                  <View style={[styles.backupDot, { 
                    backgroundColor: selectedPhoto.backupStatus === 'synced' ? '#10b981' : 
                                    selectedPhoto.backupStatus === 'pending' ? '#f59e0b' : '#ef4444' 
                  }]} />
                  <Text style={styles.backupText}>
                    {selectedPhoto.backupStatus === 'synced' ? 'Synced to Cloud' : 
                     selectedPhoto.backupStatus === 'pending' ? 'Pending Sync' : 'Sync Failed'}
                  </Text>
                </View>
              </View>
            </BlurView>
  ...
  return (
    <View style={[styles.container, { backgroundColor: isDark ? '#000' : '#f8fafc' }]}>
      ...
      {/* Vault Lock Modal - Uses existing security system */}
      <VaultLockModal
      ...
      <BatchSelectBar
        ...
        onCreateStory={selectedPhotos.size >= 2 ? () => handleCreateStory(Array.from(selectedPhotos)) : undefined}
        isDark={isDark}
      />

      {/* Floating Camera Button - Centered bottom, hides on scroll */}
      ...
      >
          ...
          <View style={styles.headerTop}>
            ...
            <View style={styles.headerTitleContainer}>
              <Text style={[styles.headerTitle, { color: isDark ? '#fff' : '#1e293b' }]}>
                {currentBaby?.name ? `${currentBaby.name}'s Photos` : 'Gallery'}
              </Text>
              {activeAlbum && (
            ...
            <View style={styles.headerActions}>
              {/* Vault Toggle */}
              <TouchableOpacity 
                style={[styles.iconButton, vaultEnabled && styles.iconButtonActive]}
                onPress={toggleVault}
              >
                <Ionicons 
                  name={vaultEnabled ? "shield" : "shield-outline"} 
                  size={22} 
                  color={vaultEnabled ? "#ef4444" : isDark ? '#fff' : '#1e293b'} 
                />
              </TouchableOpacity>
              
              {/* Batch Mode Toggle */}
              <TouchableOpacity 
                style={[styles.iconButton, isBatchMode && styles.iconButtonActive]}
                onPress={() => {
              ...
              >
                <Ionicons 
                  ...
                  size={24} 
                  color={isBatchMode ? "#667eea" : isDark ? '#fff' : '#1e293b'} 
                />
              </TouchableOpacity>
          ...
          >
            ...
            >
              <View style={styles.babyProfileContent}>
                <Text style={styles.babyProfileEmoji}>{currentBaby.avatar || '👶'}</Text>
                <View style={styles.babyProfileInfo}>
                  <Text style={styles.babyProfileName}>{currentBaby.name}</Text>
                  <Text style={styles.babyProfileAge}>{currentBaby.age}</Text>
                </View>
        ...
        <View style={styles.gridHeader}>
          <Text style={[styles.gridTitle, { color: isDark ? '#fff' : '#1e293b' }]}>
            {filteredPhotos.length} Photos
          </Text>
          <View style={styles.gridActions}>
            <TouchableOpacity 
              style={styles.gridAction}
              onPress={() => handleCreateStory(Array.from(selectedPhotos))}
              disabled={selectedPhotos.size < 2}
            >
              <Ionicons 
                name="images" 
                size={18} 
                color={selectedPhotos.size >= 2 ? "#667eea" : "#94a3b8"} 
              />
              <Text style={[
                styles.gridActionText,
                selectedPhotos.size >= 2 && styles.gridActionTextActive
              ]}>
                Create Story
              </Text>
            </TouchableOpacity>
        ...
        {viewMode === 'grid' ? (
          <View style={styles.photoGrid}>
            {groupedPhotos.map((group) => (
              <View key={group.title}>
                <DateGroupHeader date={group.title} isDark={isDark} />
                <View style={styles.gridRow}>
                  {group.data.map((item, index) => (
                    <View key={item.id} style={{ width: ITEM_SIZE, marginBottom: SPACING }}>
          ...
          <View style={styles.listContainer}>
            {filteredPhotos.map((item, index) => renderPhoto({ item, index }))}
          </View>
        )}
        ...
        {filteredPhotos.length === 0 && (
          <View style={styles.emptyContainer}>
            ...
            <TouchableOpacity style={styles.captureButton} onPress={handleOpenCamera}>
              ...
              <Text style={styles.captureButtonText}>Capture First Photo</Text>
            </TouchableOpacity>
          </View>
        )}

        {/* Bottom padding for floating camera button */}
      ...
      >
        <View style={[styles.linkModalContainer, { backgroundColor: isDark ? 'rgba(0,0,0,0.9)' : 'rgba(255,255,255,0.95)' }]}>
          <BlurView intensity={80} style={StyleSheet.absoluteFill} tint={isDark ? 'dark' : 'light'} />
          <View style={styles.linkModalContent}>
            <Text style={[styles.linkModalTitle, { color: isDark ? '#fff' : '#1e293b' }]}>
            ...
            >
              <LinearGradient colors={['#667eea', '#764ba2']} style={styles.linkOptionIcon}>
                <Ionicons name="trending-up" size={24} color="#fff" />
              </LinearGradient>
              <View style={styles.linkOptionText}>
                <Text style={[styles.linkOptionTitle, { color: isDark ? '#fff' : '#1e293b' }]}>
                  Link to Growth Entry
                </Text>
                <Text style={styles.linkOptionDesc}>Attach to height/weight measurement</Text>
              </View>
            ...
            <TouchableOpacity 
              style={styles.linkOption}
              onPress={() => handleLinkToEntry(selectedPhoto!, 'milestone', 'ms1')}
            >
              <LinearGradient colors={['#f59e0b', '#fbbf24']} style={styles.linkOptionIcon}>
              ...
              <View style={styles.linkOptionText}>
                ...
                <Text style={styles.linkOptionDesc}>Mark as achievement photo</Text>
              </View>
              <Ionicons name="chevron-forward" size={20} color="#94a3b8" />
            </TouchableOpacity>
            ...
            <TouchableOpacity 
              style={styles.linkOption}
              onPress={() => handleLinkToEntry(selectedPhoto!, 'activity', 'act1')}
            >
              <LinearGradient colors={['#10b981', '#34d399']} style={styles.linkOptionIcon}>
              ...
              <View style={styles.linkOptionText}>
                ...
                <Text style={styles.linkOptionDesc}>Connect to feeding/sleep/potty log</Text>
              </View>
              <Ionicons name="chevron-forward" size={20} color="#94a3b8" />
            </TouchableOpacity>
      ...
      >
        <View style={styles.storyModalContainer}>
          ...
          <View style={styles.storyModalContent}>
            ...
            <Text style={styles.storyModalText}>Creating your story...</Text>
            <Text style={styles.storyModalSubtext}>Collage magic in progress ✨</Text>
          </View>
        </View>
      </Modal>
    </View>
...
const styles = StyleSheet.create({
  ...
  // Floating Camera Button - Centered bottom
  ...
  // Photo Modal - Enhanced Glassmorphism
  modalContainer: {
    flex: 1,
