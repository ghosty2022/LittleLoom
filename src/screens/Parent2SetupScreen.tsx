...
const ConfirmModal = ({ visible, title, message, onConfirm, onCancel, type = 'default', isDark }: any) => {
  if (!visible) return null;
  const colors = type === 'danger' ? ['#ef4444', '#dc2626'] : ['#667eea', '#764ba2'];
...
export default function Parent2SetupScreen({ navigation }: Props) {
  ...
  const handleAddParent = useCallback(async () => {
    ...
    try {
      ...
      // CRITICAL FIX: Navigate to BabyOptional after adding parent
      setTimeout(() => navigation.replace('BabyOptional'), 1500);
    } catch (error) {
      showToast('error', 'Error', 'Failed to send invitation');
      setIsLoading(false);
    }
  }, [fullName, email, inviteMember, completeSetup, navigation, showToast]);
  ...
  const handleSkip = useCallback(() => {
    ...
    setConfirmModal({
      ...
      onConfirm: async () => {
        try {
          // Don't call skipSetup here since we're in the middle of adding
          // Just navigate to BabyOptional
  ...
  return (
    <View style={styles.container}>
              ...
              <BlurView intensity={60} style={styles.glassCard}>
                ...
                <View style={[styles.inputContainer, isDark && styles.inputContainerDark]}>
                  ...
                  <TextInput
                    ...
                    placeholderTextColor={isDark ? '#64748b' : 'rgba(102,126,234,0.6)'}
                    value={fullName}
                ...
                <View style={styles.infoContainer}>
                  ...
                  <Text style={[styles.infoText, isDark && { color: '#94a3b8' }]}>An invitation will be sent to this email address</Text>
                </View>
                ...
                <TouchableOpacity style={styles.addButton} onPress={handleAddParent} disabled={isLoading}>
                  <LinearGradient colors={['#667eea', '#764ba2']} style={styles.addGradient}>
                    {isLoading ? <ActivityIndicator color="#fff" /> : <><Ionicons name="send" size={20} color="#fff" /><Text style={styles.addText}>Send Invitation</Text></>}
                  </LinearGradient>
                </TouchableOpacity>
      ...
      <ConfirmModal {...confirmModal} onCancel={() => setConfirmModal({ ...confirmModal, visible: false })} isDark={isDark} />
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  alertContainer: { flexDirection: 'row', alignItems: 'center', borderRadius: 16, padding: 16, shadowColor: '#000', shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.3, shadowRadius: 20, elevation: 10, minWidth: 300, maxWidth: 360 },
  ...
  confirmModal: { width: 320, borderRadius: 24, padding: 24, alignItems: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 20 }, shadowOpacity: 0.3, shadowRadius: 40, elevation: 20 },
  ...
  confirmMessage: { fontSize: 14, color: '#64748b', textAlign: 'center', marginBottom: 24 },
  ...
  glassCard: { borderRadius: 24, padding: 28, backgroundColor: 'rgba(255,255,255,0.8)' },
  formTitle: { fontSize: 24, fontWeight: '700', color: '#1a1a1a', marginBottom: 24, textAlign: 'center' },
  
  inputContainer: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(102,126,234,0.08)', borderRadius: 16, paddingHorizontal: 16, marginBottom: 16, height: 56, borderWidth: 1, borderColor: 'rgba(102,126,234,0.15)' },
  inputContainerDark: { backgroundColor: 'rgba(30,30,40,0.6)', borderColor: 'rgba(255,255,255,0.1)' },
  inputIcon: { marginRight: 12 },
  input: { flex: 1, fontSize: 16, color: '#1a1a1a' },
  
  infoContainer: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 24, paddingHorizontal: 4 },
  infoText: { flex: 1, fontSize: 13, color: '#667eea' },
  
  addButton: { borderRadius: 16, overflow: 'hidden', marginBottom: 16 },
  addGradient: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 16, gap: 10 },
