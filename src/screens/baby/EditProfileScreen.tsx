...
import { View, Text, StyleSheet, TouchableOpacity, TextInput, Alert, useColorScheme, Dimensions, Modal, Platform, Image, ActivityIndicator, RefreshControl, Pressable, StatusBar  } from 'react-native';
import { LinearGradient } from 'expo-linear-gradient';
import { BlurView } from 'expo-blur';
import { Ionicons } from '@expo/vector-icons';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import DateTimePicker, { DateTimePickerEvent } from '@react-native-community/datetimepicker';
import Animated, { FadeInUp, FadeInDown, Layout, useSharedValue, useAnimatedStyle, interpolate, Extrapolate, useAnimatedScrollHandler, withSpring, withTiming, runOnJS, SlideInUp, SlideOutDown } from 'react-native-reanimated';
...
export default function EditProfileScreen({ navigation, route }: EditProfileScreenProps) {
  ...
  const renderHealthForm = () => (
    <Animated.View entering={FadeInUp} style={styles.tabPanel}>
      ...
      <GlassmorphismCard style={styles.actionCard} intensity={80} delay={200} onPress={() => navigation.navigate('UniversalTracker' as never, { type: 'medication' } as never)}>
      ...
      <GlassmorphismCard style={styles.actionCard} intensity={80} delay={300} onPress={() => navigation.navigate('UniversalTracker' as never, { type: 'sleep' } as never)}>
        <View style={styles.actionRow}>
      ...
      <GlassmorphismCard style={styles.actionCard} intensity={80} delay={400} onPress={() => navigation.navigate('UniversalTracker' as never, { type: 'feed' } as never)}>
        <View style={styles.actionRow}>
      ...
      <GlassmorphismCard style={styles.actionCard} intensity={80} delay={500} onPress={() => navigation.navigate('GrowthChart' as never, { babyId: currentBabyData?.id } as never)}>
        <View style={styles.actionRow}>
  ...
  const renderOverview = () => (
    <Animated.View entering={FadeInUp} style={styles.tabPanel}>
      ...
      <View style={styles.sectionHeader}>
        ...
        <TouchableOpacity style={styles.seeAllButton} onPress={() => navigation.navigate('FamilySharing' as never)}>
          <Text style={styles.seeAllText}>Manage</Text>
          <Ionicons name="chevron-forward" size={14} color="#667eea" />
        </TouchableOpacity>
      </View>

      <GlassmorphismCard style={styles.familyCard} intensity={80} delay={300} onPress={() => navigation.navigate('FamilySharing' as never)}>
      ...
      {recentActivities.length > 0 && (
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <TouchableOpacity onPress={() => navigation.navigate('Timeline' as never)}>
          ...
          {recentActivities.map((activity, index) => (
            ...
            >
              <TouchableOpacity 
                onPress={() => navigation.navigate('UniversalTracker' as never, { type: activity.type } as never)} 
                activeOpacity={0.8}
              >
