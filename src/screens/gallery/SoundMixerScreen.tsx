...
import { FadeIn, FadeInDown, FadeInUp, Layout, useAnimatedStyle, useSharedValue, withSpring, withTiming, ZoomIn } from 'react-native-reanimated';
import { Alert, Animated, Button, Dimensions, Image, Linking, Modal, Platform, ScrollView, Settings, Share, StatusBar, StyleSheet, Text, TextInput, TouchableOpacity, View } from 'react-native';;
import { LinearGradient } from 'expo-linear-gradient';
import { BlurView } from 'expo-blur';
import { Ionicons } from '@expo/vector-icons';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import Animated, { FadeInUp, FadeIn, FadeInDown, Layout, ZoomIn, useSharedValue, useAnimatedStyle, withSpring, withTiming } from 'react-native-reanimated';
  ...
  return (
    <View style={[StyleSheet.absoluteFill, { zIndex: 1000, justifyContent: 'center', alignItems: 'center' }]} pointerEvents="box-none">
      ...
      <Animated.View style={[importStyles.modalContent, modalStyle, { backgroundColor: isDark ? '#1a1a2e' : '#fff' }]}>
        ...
        <Text style={importStyles.subtitle}>Add songs to your baby's playlist</Text>
        ...
        <TouchableOpacity style={importStyles.option} onPress={() => onImport('device')}>
          <LinearGradient colors={['#667eea', '#764ba2']} style={importStyles.gradient}>
            ...
            <View style={importStyles.textContainer}>
              <Text style={importStyles.optionTitle}>Device</Text>
              <Text style={importStyles.optionDesc}>Import from your device</Text>
            </View>
    ...
    <View style={playlistCardStyles.wrapper}>
      <TouchableOpacity onPress={onPress} activeOpacity={0.8}>
        ...
        <View style={playlistCardStyles.textContainer}>
          ...
          <Text style={playlistCardStyles.desc}>{playlist.description}</Text>
        </View>
      </TouchableOpacity>
    </View>
  );
...
export default function SoundMixerScreen({ navigation }: SoundMixerScreenProps) {
  ...
  const { themeColors, avatar, settings: customSettings, isLoaded: customLoaded } = useCustomization();
  ...
  const searchBlurStyle = useMemo(() => ({
    borderColor: customLoaded ? `${themeColors.primary}30` : 'rgba(255,255,255,0.1)'
  }), [customLoaded, themeColors]);

  const tabActiveStyle = useMemo(() => ({
    backgroundColor: customLoaded ? `${themeColors.primary}30` : 'rgba(255,255,255,0.15)',
    borderColor: customLoaded ? `${themeColors.primary}60` : 'rgba(255,255,255,0.3)'
  }), [customLoaded, themeColors]);

  const tabTextActiveStyle = useMemo(() => ({
    color: customLoaded ? themeColors.primary : '#fff'
  }), [customLoaded, themeColors]);

  const scrollContentPadding = useMemo(() => ({
    paddingTop: insets.top + (customLoaded && customSettings.compactView ? 10 : 20),
  ...
  useEffect(() => {
    const smartPlaylists: Playlist[] = [
      ...
      },
      {
        ...
        coverColor: customLoaded ? themeColors.primary : '#667eea',
  ...
  const moods = useMemo(() => [
    { id: 'sleep', label: 'Sleep', emoji: '😴', color: customLoaded ? themeColors.primary : '#667eea' },
    { id: 'calm', label: 'Calm', emoji: '😌', color: customLoaded ? themeColors.secondary : '#11998e' },
    { id: 'play', label: 'Play', emoji: '🎮', color: '#feca57' },
    { id: 'focus', label: 'Focus', emoji: '🎯', color: customLoaded ? themeColors.accent : '#ff6b6b' },
  ], [customLoaded, themeColors]);
  ...
  return (
    <View style={styles.container}>
      ...
      <LinearGradient
        colors={customLoaded ? themeColors.colors : ['#0f0f1e', '#1a1a2e', '#16213e']}
        ...
        <View style={styles.header}>
          <View style={styles.headerText}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
              <Text style={{ fontSize: 20 }}>{customLoaded ? avatar : '👶'}</Text>
        ...
        {showSearch && (
          <FadeInDown>
            <View style={styles.searchContainer}>
              <BlurView intensity={80} style={[styles.searchBlurBase, searchBlurStyle]} tint="dark">
        ...
        <View style={styles.tabsContainer}>
          <AutoHideScrollView horizontal showsHorizontalScrollIndicator={false}>
            {['all', 'favorites', 'imported'].map((tab) => (
              <TouchableOpacity
                key={tab}
                style={[styles.tabBase, activeTab === tab && [styles.tabActiveBase, tabActiveStyle]]}
              ...
              >
                <Ionicons
                  ...
                  style={{ marginRight: 6 }}
                />
                <Text style={[styles.tabTextBase, activeTab === tab && [styles.tabTextActiveBase, tabTextActiveStyle]]}>
...
const styles = StyleSheet.create({
  ...
  searchBlurBase: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 14, borderRadius: 20, backgroundColor: 'rgba(255,255,255,0.05)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)' },
  ...
  tabBase: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 10, borderRadius: 20, marginRight: 10, backgroundColor: 'rgba(255,255,255,0.05)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)' },
  tabActiveBase: {},
  tabTextBase: { fontSize: 14, fontWeight: '600', color: '#94a3b8' },
  tabTextActiveBase: {},
