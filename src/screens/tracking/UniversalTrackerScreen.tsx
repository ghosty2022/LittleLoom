...
const useTrackerTheme = () => {
  ...
  }), [isDark, primary, secondary, reduceMotion, avatar, themeColors, colors, appIsDark]);
...
const CustomTrackerManager: React.FC<{
  ...
  const colorOptions = [
    theme.primary, theme.secondary, '#11998e', '#43e97b', '#ff6b6b',
    ...
    return (
      <Modal visible={visible} transparent animationType="slide">
        <View style={[styles.modalOverlay, { backgroundColor: theme.isDark ? 'rgba(0,0,0,0.6)' : 'rgba(0,0,0,0.4)' }]}>
          <BlurView intensity={95} style={[styles.modalContent, { maxHeight: '85%', backgroundColor: theme.isDark ? 'rgba(20,20,30,0.98)' : 'rgba(255,255,255,0.98)' }]}>
            ...
            <AutoHideScrollView showsVerticalScrollIndicator={false}>
              <View style={styles.fieldContainer}>
                <Text style={[styles.fieldLabel, { color: theme.text.primary }]}>Tracker Name *</Text>
                <TextInput
              ...
              <View style={styles.fieldContainer}>
                <Text style={[styles.fieldLabel, { color: theme.text.primary }]}>Emoji</Text>
                <TextInput
                  style={[styles.textInput, { fontSize: 24, color: theme.text.primary, backgroundColor: theme.surface.card, borderColor: theme.surface.border }]}
                  placeholder="📊"
                  placeholderTextColor={theme.text.muted}
                  maxLength={2}
              ...
              <View style={styles.fieldContainer}>
                <Text style={[styles.fieldLabel, { color: theme.text.primary }]}>Color Theme</Text>
              ...
              <View style={styles.fieldContainer}>
                <Text style={[styles.fieldLabel, { color: theme.text.primary }]}>Icon</Text>
                <View style={styles.iconGrid}>
                  {iconOptions.map((icon) => (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.iconOption,
                        { backgroundColor: theme.surface.card, borderColor: theme.surface.border },
                        formData.icon === icon && { backgroundColor: formData.color, borderColor: formData.color }
                      ]}
                      onPress={() => setFormData({ ...formData, icon })}
                    >
                      <Ionicons name={icon as any} size={24} color={formData.icon === icon ? '#fff' : theme.text.secondary} />
                    </TouchableOpacity>
                  ))}
                </View>
              </View>
              ...
              <TouchableOpacity style={styles.saveButton} onPress={handleSave}>
                <LinearGradient colors={formData.gradient || [theme.primary, theme.secondary]} style={styles.saveGradient}>
              ...
              >
                <View style={[styles.saveGradient, { backgroundColor: theme.surface.border }]}>
                  <Text style={[styles.saveText, { color: theme.text.secondary }]}>Cancel</Text>
                </View>
              </TouchableOpacity>
            </AutoHideScrollView>
          </BlurView>
  ...
  return (
    <Modal visible={visible} transparent animationType="slide">
      <View style={[styles.modalOverlay, { backgroundColor: theme.isDark ? 'rgba(0,0,0,0.6)' : 'rgba(0,0,0,0.4)' }]}>
        <BlurView intensity={95} style={[styles.modalContent, { backgroundColor: theme.isDark ? 'rgba(20,20,30,0.98)' : 'rgba(255,255,255,0.98)' }]}>
          <View style={styles.modalHeader}>
            <Text style={[styles.modalTitle, { color: theme.text.primary }]}>🎨 Custom Trackers</Text>
          ...
          >
            <LinearGradient colors={[theme.primary, theme.secondary]} style={styles.addTrackerGradient}>
              <Ionicons name="add" size={24} color="#fff" />
              <Text style={styles.addTrackerText}>Create New Tracker</Text>
            </LinearGradient>
          </TouchableOpacity>

          <AutoHideScrollView showsVerticalScrollIndicator={false} style={{ maxHeight: 400 }}>
            {customTrackers.map((tracker) => (
              <View key={tracker.id} style={[styles.trackerListItem, { backgroundColor: theme.surface.card, borderColor: theme.surface.border }]}>
                <View style={[styles.trackerIcon, { backgroundColor: `${tracker.color}20` }]}>
                  <Text style={{ fontSize: 24 }}>{tracker.emoji}</Text>
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={[styles.trackerName, { color: theme.text.primary }]}>{tracker.name}</Text>
                  <Text style={styles.trackerMeta}>Custom • {tracker.fields.length} fields</Text>
                </View>
                <View style={styles.trackerActions}>
                  <TouchableOpacity
                    style={[styles.trackerActionBtn, { backgroundColor: theme.surface.card, borderColor: theme.surface.border }]}
            ...
            {customTrackers.length === 0 && (
              <View style={styles.emptyCustomTrackers}>
                <Text style={[styles.emptyCustomTrackersText, { color: theme.text.muted }]}>No custom trackers yet</Text>
                <Text style={[styles.emptyCustomTrackersSub, { color: theme.text.secondary }]}>Create your own tracking categories!</Text>
              </View>
            )}
          </AutoHideScrollView>
        </BlurView>
  ...
  const renderField = (field: FieldConfig) => {
    ...
    switch (field.type) {
      ...
      case 'number':
        return (
          <View key={field.id} style={styles.fieldContainer}>
            <Text style={[styles.fieldLabel, { color: theme.text.primary }]}>{field.label}{field.required && ' *'}</Text>
            <TextInput
              style={[styles.textInput, {
                color: theme.text.primary,
                backgroundColor: theme.surface.card,
            ...
            <View style={styles.optionsGrid}>
              {field.options?.map((option) => (
                ...
                >
                  ...
                  {option.icon && <Ionicons name={option.icon as any} size={20} color={value === option.id ? tracker.color : theme.text.secondary} />}
                  <Text style={[
                    styles.optionLabel,
      ...
      case 'toggle':
        return (
          <View key={field.id} style={styles.fieldContainer}>
            <View style={[styles.toggleContainer, { backgroundColor: theme.surface.card }]}>
              <Text style={[styles.fieldLabel, { color: theme.text.primary, marginBottom: 0 }]}>{field.label}</Text>
              <Switch
                value={value || false}
                onValueChange={(val) => {
                  Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                ...
                trackColor={{ false: theme.surface.border, true: `${tracker.color}50` }}
              ...
              {[1, 2, 3, 4, 5].map((star) => (
                ...
                >
                  <Ionicons
                    ...
                    size={32}
                    color={star <= (value || 0) ? '#ffd700' : theme.surface.border}
                  />
                </TouchableOpacity>
              ))}
            </View>
      ...
      case 'photo':
        return (
          <View key={field.id} style={styles.fieldContainer}>
            <Text style={[styles.fieldLabel, { color: theme.text.primary }]}>{field.label}</Text>
            <TouchableOpacity style={[styles.photoButton, { borderColor: theme.surface.border }]}>
              <Ionicons name="camera-outline" size={24} color={tracker.color} />
              <Text style={[styles.photoButtonText, { color: theme.text.secondary }]}>Add Photo</Text>
            </TouchableOpacity>
          </View>
        );
  ...
  return (
    ...
    >
      <KeyboardAvoidingView
        ...
        style={[styles.modalOverlay, { backgroundColor: theme.isDark ? 'rgba(0,0,0,0.6)' : 'rgba(0,0,0,0.4)' }]}
      >
        <BlurView intensity={95} style={[styles.modalContent, { backgroundColor: theme.isDark ? 'rgba(20,20,30,0.98)' : 'rgba(255,255,255,0.98)' }]}>
          <View style={styles.modalHeader}>
            <View>
              <Text style={[styles.modalTitle, { color: theme.text.primary }]}>{tracker.emoji} {tracker.name}</Text>
              <Text style={[styles.modalSubtitle, { color: theme.text.secondary }]}>For {babyName}</Text>
            </View>
            <TouchableOpacity onPress={onClose} style={styles.modalClose}>
              <Ionicons name="close" size={24} color={theme.text.secondary} />
            </TouchableOpacity>
          ...
          {errors.length > 0 && (
            <View style={[styles.errorsContainer, { backgroundColor: theme.isDark ? 'rgba(239,68,68,0.15)' : '#fef2f2', borderLeftColor: '#ef4444' }]}>
          ...
          <AutoHideScrollView showsVerticalScrollIndicator={false} style={styles.modalScroll}>
    ...
    >
      <View style={styles.entryRow}>
        ...
        <TouchableOpacity style={styles.entryCardContainer} activeOpacity={0.9}>
          <BlurView intensity={theme.isDark ? 40 : 70} style={[styles.entryCard, { backgroundColor: theme.surface.bg, borderColor: theme.surface.border }]}>
            ...
            <View style={styles.entryContent}>
              ...
              {entry.details && (
                <Text style={[styles.entrySubtitle, { color: theme.text.secondary }]} numberOfLines={2}>{entry.details}</Text>
              )}
              <Text style={[styles.timestampText, { color: theme.text.muted }]}>
            ...
            <View style={styles.entryActions}>
              <TouchableOpacity style={[styles.actionButton, { backgroundColor: theme.surface.card }]} onPress={onEdit}>
                <Ionicons name="create-outline" size={18} color={theme.text.muted} />
              </TouchableOpacity>
              <TouchableOpacity style={[styles.actionButton, { backgroundColor: theme.surface.card }]} onPress={onDelete}>
...
const StickyHeader: React.FC<{
  ...
  return (
    <View style={styles.stickyHeaderContainer}>
      <LinearGradient
        colors={theme.isDark
          ? [tracker.gradient[0] + '15', tracker.gradient[1] + '08', '#0f0f1e']
          : [tracker.gradient[0] + '20', tracker.gradient[1] + '10', '#fff']
        }
      ...
      <View style={styles.headerContent}>
        <TouchableOpacity onPress={onBack} style={styles.backButton}>
          <BlurView intensity={theme.isDark ? 40 : 80} style={[styles.backBlur, { backgroundColor: theme.isDark ? 'rgba(30,30,40,0.6)' : 'rgba(255,255,255,0.8)' }]}>
            <Ionicons name="arrow-back" size={24} color={theme.text.primary} />
          </BlurView>
        </TouchableOpacity>

        <Animated.View style={[styles.headerCenter, titleAnimatedStyle]}>
          <Text style={[styles.headerTitleLarge, { color: theme.text.primary }]}>{tracker.emoji} {tracker.name}</Text>
          <Text style={[styles.headerSubtitle, { color: theme.text.secondary }]}>{babyName} • {stats.primary?.value || 0} today</Text>
        </Animated.View>

        <View style={styles.headerRightActions}>
          <TouchableOpacity onPress={onManageTrackers} style={styles.headerActionBtn}>
            <BlurView intensity={theme.isDark ? 40 : 80} style={[styles.actionBlur, { backgroundColor: theme.isDark ? 'rgba(30,30,40,0.6)' : 'rgba(255,255,255,0.8)' }]}>
              <Ionicons name="grid-outline" size={22} color={theme.text.primary} />
            </BlurView>
          </TouchableOpacity>
      ...
      <Animated.View style={[styles.stickyTitleContainer, headerAnimatedStyle]}>
        <BlurView intensity={theme.isDark ? 40 : 90} style={[styles.stickyBlur, { backgroundColor: theme.isDark ? 'rgba(20,20,30,0.9)' : 'rgba(255,255,255,0.9)' }]}>
          <Text style={[styles.stickyTitle, { color: theme.text.primary }]}>{tracker.emoji} {tracker.name}</Text>
          <Text style={[styles.stickySubtitle, { color: theme.text.secondary }]}>{stats.primary?.value || 0} {stats.primary?.label || 'entries'}</Text>
        </BlurView>
      </Animated.View>
    </View>
  );
...
export default function UniversalTrackerScreen() {
  ...
  const calculateStats = useCallback((trackerId: string, allEntries: ActivityEntry[]) => {
    ...
    switch (trackerId) {
      ...
      case 'feed':
        const todayAmount = today.reduce((sum, e) => sum + (parseFloat(e.amount || '0') || 0), 0);
        return {
      ...
      case 'growth':
        return {
          ...
          secondary: [
            ...
            { label: 'Types', value: [...new Set(typeEntries.map(e => e.measurementType).filter(Boolean))].length, emoji: '📊' },
          ]
        };
      ...
      case 'milestone':
        return {
          ...
          secondary: [
            ...
            { label: 'Categories', value: [...new Set(typeEntries.map(e => e.milestoneType).filter(Boolean))].length, emoji: '📂' },
          ]
        };

      default:
        return {
          primary: { label: 'Entries Today', value: today.length, emoji: currentTracker?.emoji || '📊', color: currentTracker?.color || theme.primary },
          secondary: [
            { label: 'This Week', value: week.length, emoji: '📅' },
            { label: 'Total', value: typeEntries.length, emoji: '📊' },
  ...
  }, [addEntry, currentTrackerId, currentBaby, userProfile, theme.reduceMotion]);
  ...
  }, [navigation, currentTrackerId, theme.reduceMotion]);
  ...
  }, [deleteEntry, currentTracker, theme.reduceMotion]);
    ...
    return (
      <LinearGradient colors={theme.bgColors} style={styles.container}>
        ...
        <View style={styles.loadingContainer}>
          <Text style={[styles.loadingText, { color: theme.text.secondary }]}>Loading entries...</Text>
        </View>
      </LinearGradient>
    );
  }
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.isDark ? '#0f0f1e' : '#fff' }]}>
      <StatusBar style={theme.statusBar} />
      ...
      >
        <Animated.View entering={theme.reduceMotion ? undefined : FadeInUp.delay(100)} style={styles.activitySelectorContainer}>
          <AutoHideScrollView
            horizontal
        ...
        <Animated.View entering={theme.reduceMotion ? undefined : FadeInUp.delay(200)} style={styles.statsContainer}>
          <AutoHideScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.statsContent}>
            ...
            {stats.secondary?.map((stat: any, idx: number) => (
              <View key={idx} style={[styles.statCard, styles.secondaryStatCard, { backgroundColor: theme.surface.bg, borderColor: theme.surface.border }]}>
                ...
                <Text style={[styles.secondaryStatLabel, { color: theme.text.muted }]}>{stat.label}</Text>
              </View>
            ))}
          </AutoHideScrollView>
        </Animated.View>

        <Animated.View entering={theme.reduceMotion ? undefined : FadeInUp.delay(300)} style={styles.filterContainer}>
          <View style={[styles.timeRangeTabs, { backgroundColor: theme.surface.card, borderColor: theme.surface.border }]}>
        ...
        <View style={styles.timelineContainer}>
          {groupedEntries.length === 0 ? (
            <Animated.View entering={theme.reduceMotion ? undefined : FadeInUp.delay(400)} style={styles.emptyState}>
              ...
              <Text style={[styles.emptyTitle, { color: theme.text.primary }]}>No {currentTracker.name.toLowerCase()} entries yet</Text>
            ...
            groupedEntries.map((group, groupIndex) => (
              <View key={`${group.title}-${groupIndex}`} style={styles.daySection}>
                <Animated.View entering={theme.reduceMotion ? undefined : FadeInUp.delay(groupIndex * 100)}>
